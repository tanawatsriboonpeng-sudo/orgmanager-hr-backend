...
console.log('[EmailService] RESEND_API_KEY:', process.env.RESEND_API_KEY ? `SET (${process.env.RESEND_API_KEY.length} chars)` : 'MISSING')

const resend = new Resend(process.env.RESEND_API_KEY)
const FROM_EMAIL = 'สิริคอนส์ HR <onboarding@resend.dev>'
...
const sendOTPEmail = async (to, name, otp) => {
  ...
  try {
    ...
    if (error) {
      console.error('[sendOTPEmail] FAILED:', error.message || JSON.stringify(error))
      throw new Error(error.message || 'Email send failed')
    }
    console.log('[sendOTPEmail] SUCCESS:', data.id)
  } catch (err) {
